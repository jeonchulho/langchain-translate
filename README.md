...
- 오버랩 문맥 기반 자연 연결
	- 이전 청크의 오버랩 suffix를 문장 경계 기준으로 잘라 컨텍스트로만 사용
	- 현재 청크는 중복 prefix를 제거한 본문만 번역해 중복 출력을 방지
	- auto_chunk_overlap 옵션으로 chunk_size 기반 유효 오버랩 자동 조절
- 옵션 기반 청크 병렬 번역(asyncio.gather + 동시성 제한)
	- parallel_chunk_translation=true 일 때 max_concurrency 만큼 병렬 처리
	- parallel_context_strategy=glossary-only 일 때 용어집만 컨텍스트로 사용
	- use_fidelity_first_preset=true 이면 원문 충실도 우선 스타일을 적용하고 reasoning/retry를 끈다
- 문서 타입 기반 스타일 프리셋
...
curl -X POST http://localhost:8000/translate \
	...
	-d '{
		...
		"chunk_size": 3500,
		"chunk_overlap": 350,
		"auto_chunk_overlap": false,
		"auto_chunk_overlap_ratio": 0.15,
		"auto_chunk_overlap_max": 450,
		...
		"use_fidelity_first_preset": false,
...
출력 파일:
- 데모 모드: `examples/rotary_constitution_en_output_demo_standard_serial_<provider>.txt`
- 실제 모드: `examples/rotary_constitution_en_output_real_standard_serial_<provider>.txt`
- 원문 충실도 + 병렬(glossary-only): `examples/rotary_constitution_en_output_real_fidelity_parallel_<provider>.txt`
...
오버랩 중복 제거 before/after 비교(데모 전용):

```bash
python examples/translate_file_example.py --compare-overlap-demo --chunk-overlap 180
```
...
원문 충실도 우선 프리셋과 glossary-only 병렬 문맥을 함께 쓰려면:

```bash
python examples/translate_file_example.py --real-llm --provider ollama --model llama3.1:8b --fidelity-first --parallel --parallel-context-strategy glossary-only
```

자동 오버랩 튜닝을 함께 쓰려면:

```bash
python examples/translate_file_example.py --real-llm --auto-overlap --auto-overlap-ratio 0.12 --auto-overlap-max 240
```
